...
{
    CubismLogInfo("[GenerateShaders] START");
    
    if (_shaderSets.GetSize() > 0)
    {
        CubismLogInfo("[GenerateShaders] Already initialized, skipping");
        return;
    }
...
#ifdef CSM_TARGET_ANDROID_ES2
    CubismLogInfo("[GenerateShaders] Android ES2 mode, s_extMode=%d", s_extMode);
    if (s_extMode)
    {
        CubismLogInfo("[GenerateShaders] Loading Tegra shaders...");
    ...
    {
        CubismLogInfo("[GenerateShaders] Loading standard ES2 shaders...");
        _shaderSets[ShaderNames_Copy]->ShaderProgram = LoadShaderProgramFromFile("VertShaderSrcCopy.vert", "FragShaderSrcCopy.frag");
        CubismLogInfo("[GenerateShaders] Copy shader: %u", _shaderSets[ShaderNames_Copy]->ShaderProgram);
        ...
        CubismLogInfo("[GenerateShaders] SetupMask shader: %u", _shaderSets[ShaderNames_SetupMask]->ShaderProgram);

        _shaderSets[ShaderNames_Normal]->ShaderProgram = LoadShaderProgramFromFile("VertShaderSrc.vert", "FragShaderSrc.frag");
        CubismLogInfo("[GenerateShaders] Normal shader: %u", _shaderSets[ShaderNames_Normal]->ShaderProgram);
        ...
        CubismLogInfo("[GenerateShaders] NormalMasked shader: %u", _shaderSets[ShaderNames_NormalMasked]->ShaderProgram);
    ...
    CubismLogInfo("[GenerateShaders] COMPLETE, total shaders=%d", _shaderSets.GetSize());
}
...
{
    CubismLogInfo("[LoadShaderProgramFromFile] vert=%s, frag=%s", vertShaderPath, fragShaderPath);
    ...
    CubismLogInfo("[LoadShaderProgramFromFile] Loading vertex shader...");
    csmByte* vertSrc = fileLoader(vertShaderPath, &vertSrcSize);
    if (vertSrc == NULL)
    {
        CubismLogError("Failed to load vertex shader: %s", vertShaderPath);
        return 0;
    }
    CubismLogInfo("[LoadShaderProgramFromFile] Vertex shader loaded: %d bytes", vertSrcSize);

    CubismLogInfo("[LoadShaderProgramFromFile] Loading fragment shader...");
    csmByte* fragSrc = fileLoader(fragShaderPath, &fragSrcSize);
    if (fragSrc == NULL)
    {
        CubismLogError("Failed to load fragment shader: %s", fragShaderPath);
        bytesReleaser(vertSrc);
        return 0;
    }
    CubismLogInfo("[LoadShaderProgramFromFile] Fragment shader loaded: %d bytes", fragSrcSize);
    ...
    // シェーダーオブジェクトを作成
    CubismLogInfo("[LoadShaderProgramFromFile] Compiling shader program...");
    GLuint result = LoadShaderProgram(vertString.GetRawString(), fragString.GetRawString());
    CubismLogInfo("[LoadShaderProgramFromFile] Shader program created: %u", result);
    return result;
}

GLuint CubismShader_OpenGLES2::LoadShaderProgram(const csmChar* vertShaderSrc, const csmChar* fragShaderSrc)
{
    CubismLogInfo("[LoadShaderProgram] START");
    GLuint vertShader, fragShader;

    // Create shader program.
    GLuint shaderProgram = glCreateProgram();
    CubismLogInfo("[LoadShaderProgram] Program created: %u", shaderProgram);
    ...
    CubismLogInfo("[LoadShaderProgram] Vertex shader compiled: %u", vertShader);
    ...
    CubismLogInfo("[LoadShaderProgram] Fragment shader compiled: %u", fragShader);
    ...
    CubismLogInfo("[LoadShaderProgram] Program linked successfully");
    ...
    CubismLogInfo("[LoadShaderProgram] COMPLETE, program=%u", shaderProgram);
