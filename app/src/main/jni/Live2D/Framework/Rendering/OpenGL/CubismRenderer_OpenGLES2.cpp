...
{
    CubismLogInfo("[Renderer::Initialize] START model=%p, maskBufferCount=%d", (void*)model, maskBufferCount);
    ...
    CubismLogInfo("[Renderer::Initialize] Checking IsBlendModeEnabled...");
    _modelRenderTargets.Clear();
    if (model->IsBlendModeEnabled())
    {
        CubismLogInfo("[Renderer::Initialize] BlendMode enabled, creating offscreen render targets...");
        // オフスクリーンの作成
        // TextureBarrierが使えない環境では2枚作成する
        // 添え字 0 は描画先となる
        // 添え字 1 はTextureBarrierの代替用
        csmInt32 createSize = CanUseTextureBarrier() ? 1 : 2;
        CubismLogInfo("[Renderer::Initialize] createSize=%d", createSize);
        for (csmInt32 i = 0; i < createSize; ++i)
        {
            CubismLogInfo("[Renderer::Initialize] Creating render target %d/%d (%ux%u)...", i+1, createSize, _modelRenderTargetWidth, _modelRenderTargetHeight);
            CubismRenderTarget_OpenGLES2 renderTarget;
            csmBool result = renderTarget.CreateRenderTarget(_modelRenderTargetWidth, _modelRenderTargetHeight, 0);
            CubismLogInfo("[Renderer::Initialize] Render target %d created: %s", i+1, result ? "OK" : "FAIL");
            _modelRenderTargets.PushBack(renderTarget);
        }
    }
    else
    {
        CubismLogInfo("[Renderer::Initialize] BlendMode disabled, skipping offscreen render targets");
    }

    CubismLogInfo("[Renderer::Initialize] Checking IsUsingMasking...");
    if (model->IsUsingMasking())
    {
        CubismLogInfo("[Renderer::Initialize] Masking enabled, creating clipping manager...");
        _drawableClippingManager = CSM_NEW CubismClippingManager_OpenGLES2();  //クリッピングマスク・バッファ前処理方式を初期化
        CubismLogInfo("[Renderer::Initialize] Calling clipping manager Initialize...");
        _drawableClippingManager->Initialize(
            *model,
            maskBufferCount,
            CubismRenderer::DrawableObjectType_Drawable
        );
        CubismLogInfo("[Renderer::Initialize] Clipping manager initialized");

        _drawableMasks.Clear();
        for (csmInt32 i = 0; i < maskBufferCount; ++i)
        {
            CubismLogInfo("[Renderer::Initialize] Creating drawable mask %d/%d...", i+1, maskBufferCount);
            CubismRenderTarget_OpenGLES2 masks;
            csmBool result = masks.CreateRenderTarget(_drawableClippingManager->GetClippingMaskBufferSize().X, _drawableClippingManager->GetClippingMaskBufferSize().Y);
            CubismLogInfo("[Renderer::Initialize] Drawable mask %d created: %s", i+1, result ? "OK" : "FAIL");
            _drawableMasks.PushBack(masks);
        }
    }
    else
    {
        CubismLogInfo("[Renderer::Initialize] Masking disabled");
    }

    CubismLogInfo("[Renderer::Initialize] Checking IsUsingMaskingForOffscreen...");
    if (model->IsUsingMaskingForOffscreen())
    {
        CubismLogInfo("[Renderer::Initialize] Offscreen masking enabled...");
    ...
    CubismLogInfo("[Renderer::Initialize] Resizing sorted objects lists...");
    ...
    CubismLogInfo("[Renderer::Initialize] offscreenCount=%d", offscreenCount);
    ...
    CubismLogInfo("[Renderer::Initialize] Calling parent Initialize...");
    CubismRenderer::Initialize(model, maskBufferCount);  //親クラスの処理を呼ぶ

    CubismLogInfo("[Renderer::Initialize] Initializing shaders...");
    // シェーダの事前初期化
    CubismShader_OpenGLES2::GetInstance();
    CubismLogInfo("[Renderer::Initialize] COMPLETE");
}
