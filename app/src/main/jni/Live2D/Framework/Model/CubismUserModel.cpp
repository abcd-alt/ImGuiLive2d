...
{
    CubismLogInfo("[CreateRenderer] START width=%u, height=%u, maskBufferCount=%d", width, height, maskBufferCount);
    CubismLogInfo("[CreateRenderer] _model=%p", (void*)_model);
    
    if (_renderer)
    {
        CubismLogInfo("[CreateRenderer] Deleting existing renderer...");
        DeleteRenderer();
        CubismLogInfo("[CreateRenderer] Existing renderer deleted");
    }
    
    CubismLogInfo("[CreateRenderer] Calling CubismRenderer::Create...");
    _renderer = Rendering::CubismRenderer::Create(width, height);
    CubismLogInfo("[CreateRenderer] Renderer created: %p", (void*)_renderer);

    CubismLogInfo("[CreateRenderer] Calling renderer->Initialize...");
    _renderer->Initialize(_model, maskBufferCount);
    CubismLogInfo("[CreateRenderer] Initialize complete");
    
    CubismLogInfo("[CreateRenderer] COMPLETE");
}
